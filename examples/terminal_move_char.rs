...
impl GameState {
    pub fn move_player(&mut self, movement: (i16, i16)) {
        let future_pos = (
            (self.player_pos.0 as i16 + movement.0),
            (self.player_pos.1 as i16 + movement.1)
            );
    ...
    terminal::run(Config::new(), &mut |term_state: &mut State, window: &mut Window| {
        ...
        for key_event in window.key_events() {
            match key_event {
                ...
                KeyEvent::Char('h') => game_state.move_player((-2, 0)),
                KeyEvent::Char('j') => game_state.move_player((0, 1)),
                KeyEvent::Char('k') => game_state.move_player((0, -1)),
                KeyEvent::Char('l') => game_state.move_player((2, 0)),
                _ => (),
            }
        }
        ...
        Pencil::new(window.surface_mut())
            ...
            .draw_char(game_state.player_pos, 'A');
    });
}
